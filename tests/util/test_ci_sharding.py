...
def test_shard_plan_expands_rows_into_per_shard_entries():
    plan = ci_sharding.shard_plan("full", stages=_PLAN_STAGES)
...
def test_shard_plan_filters_by_substring_but_keeps_candidates():
    plan = ci_sharding.shard_plan("full", stage_filter="U250", stages=_PLAN_STAGES)
...
def test_shard_plan_filter_miss_yields_empty_shards_with_candidates():
    plan = ci_sharding.shard_plan("full", stage_filter="does-not-match", stages=_PLAN_STAGES)
    assert plan["shards"] == []
    assert plan["candidates"] == ["Sanity Build", "BNN U250"]
...
def test_shard_plan_zip_artifacts_flatten_per_row_per_board_not_per_shard():
    plan = ci_sharding.shard_plan("full", stages=_PLAN_STAGES)
    # one entry per (active row, board); the 2-shard row contributes once
...
def test_shard_plan_only_includes_active_rows():
    plan = ci_sharding.shard_plan("sanity", stages=_PLAN_STAGES)
...
def test_shard_plan_matches_live_stages_stash_names():
    ...
    plan = ci_sharding.shard_plan("full")
    for entry in plan["shards"]:
        expected = ci_sharding.stash_name(
...
def test_marker_safe_pattern_accepts_live_markers_and_rejects_foot_guns():
    # MARKER_SAFE_PATTERN now has one home (ci_sharding); marker safety is
    # enforced in validate_config during Validate, so the Jenkinsfile no
    # longer carries a duplicate regex. The plugin's marker_tokens treats
    # every whitespace-separated token as an `or` disjunct, so the pattern
    # must forbid and/not and the double-space shape (empty token).
    pattern = ci_sharding.MARKER_SAFE_PATTERN
...
def test_validate_config_single_invocation_returns_full_payload(capsys):
    ...
    # The contract is "these keys present, well-formed, ready for readJSON";
    # asserted as a subset so adding a future key does not break this test.
    rc = ci_sharding.main(["validate-config", "--choice", "sanity", "--job-name", "finn.dev"])
    assert rc == 0
    captured = capsys.readouterr()
    payload = json.loads(captured.out)
    assert {"enabled_params", "retention", "job_key", "shard_plan"} <= set(payload)
    assert payload["enabled_params"] == ["sanity"]
    assert {"image", "artifact", "snapshot"} <= set(payload["retention"])
...
# ============================================================================
# hw test type discovery (hw-config-json)
# ============================================================================
...
def test_hw_config_json_cli_bundles_shards_types_and_labels(capsys):
    # The HW pipeline reads this one bundle in Validate; the contract is the
    # three keys present and well-formed for readJSON.
    rc = ci_sharding.main(["hw-config-json"])
    assert rc == 0
    parsed = json.loads(capsys.readouterr().out)
    assert {"shards", "test_types", "labels"} <= set(parsed)
    assert parsed["shards"] == ci_sharding.hw_shards()
    assert "bnn_build_sanity" in parsed["test_types"]
    assert "bnn_build_full" in parsed["test_types"]
    assert parsed["labels"] == ci_sharding.hw_test_type_labels()
...
def test_prune_pip_cache_keeps_current_and_fresh(tmp_path):
    ...
    os.utime(str(old), (1, 1))
    matched = ci_sharding.prune_pip_cache(str(root), str(keep), max_age_days=14)
    # 'old' has an ancient mtime -> pruned; 'keep' is excluded; 'recent' is fresh
    assert matched == 1
    assert not old.exists()
    assert keep.exists()
    assert fresh.exists()
...
def test_prune_pip_cache_dry_run_deletes_nothing(tmp_path):
    ...
    os.utime(str(old), (1, 1))
    matched = ci_sharding.prune_pip_cache(str(root), "", max_age_days=14, dry_run=True)
    assert matched == 1
    assert old.exists()


def test_prune_pip_cache_missing_root_is_noop(tmp_path):
    assert ci_sharding.prune_pip_cache(str(tmp_path / "absent"), "", 14) == 0
...
def test_parse_lsf_jobs_text_form_groups_by_build():
    ...
    out = ci_sharding.parse_lsf_jobs("finn_ci_finn_", raw)
...
def test_parse_lsf_jobs_ignores_non_numeric_build_token():
    ...
    assert ci_sharding.parse_lsf_jobs("finn_ci_finn_", raw) == {}


def test_parse_lsf_jobs_empty_input():
    assert ci_sharding.parse_lsf_jobs("finn_ci_finn_", "") == {}
    assert ci_sharding.parse_lsf_jobs("finn_ci_finn_", "   \n  ") == {}
