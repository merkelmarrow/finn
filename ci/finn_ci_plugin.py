...
The timing observability state lives on the registered plugin instance (not a
module global) so xdist workers and inline pytester sub-runs each carry their
own, and ``pytest_runtest_logreport`` (which has no config argument) can reach
it via ``self``.
"""
import pytest
...
def _is_xdist_worker(config):
    """True on an xdist worker. Uses ``workerinput`` (set by xdist on the worker
    config) rather than the ``PYTEST_XDIST_WORKER`` env var, so an inline
    ``pytester.runpytest`` sub-run is correctly seen as a non-worker even when
    the outer process is a worker and the env var is inherited."""
...
class _FinnCiShardingPlugin:
    """Stateful hooks. One instance per session (controller or worker); the
    timing dict is left None on xdist workers so log-reports forwarded to the
    controller are not double-counted."""
    ...
    def pytest_sessionfinish(self, session, exitstatus):
        ...
        for nodeid, duration in self.timings["per_test_seconds"].items():
            group = self.timings["nodeid_to_group"].get(nodeid, nodeid)
        ...
        payload = {
            ...
            "total_test_seconds": sum(self.timings["per_test_seconds"].values()),
