...
while IFS= read -r -d '' board_dir; do
  ...
  for model_dir in "$board_dir"/*; do
    ...
    # cp (not mv): the source lives in the per-shard build dir, which is kept
    # for failure triage and may be read again; the staged copy on NFS is the
    # one we zip. The trees are small (per-board deployment dirs).
