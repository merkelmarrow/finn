...
// given, gates the stash on that file existing (the HW form must not stash an
// empty set when a board produced no XML); allowEmpty controls the stash step.
...
// Is any agent carrying ``labelName`` online? Lives here (not in Jenkinsfile_HW)
// so both pipelines share one definition, and wraps the Jenkins API in the
// same RejectedAccessException guard the build pipeline uses for its executor
// query. On a Script Security rejection it logs the approval hint and reports
// offline; a board with a READY zip then trips the caller's offline-with-zip
// UNSTABLE path rather than crashing the whole HW stage.
boolean isNodeOnline(String labelName) {
  try {
    ...
    echo "isNodeOnline(${labelName}): Jenkins API rejected by Script Security (${e.message}). " +
         "Treating as offline; approve Jenkins.instance.getLabel/getNodes in Manage Jenkins to restore the check."
    return false
  } catch (Exception e) {
